...
const cartMiniIcon = document.querySelector(
  "span.position-absolute.top-0.start-100.translate-middle.badge.rounded-pill.bg-primary.h-pointer span",
);
...
//toast selectors
const toastTitleSelector = "strong";
const toastQtySelector = "div.text-secondary";
...
function renderShopItems(arrayItems) {
  ...
  arrayItems.forEach((product) => {
    ...
    //final price
    templateShopLi.querySelector(itemFinalPriceSelector).textContent = `$${(
      product.price -
...
function setToCart(parentItem) {
  ...
  const id = parentItem.querySelector(addToCartSelector).dataset.id;
  const title = parentItem.querySelector(itemTitleSelector).textContent;
  const thumnailUrl = parentItem.querySelector(itemImageSelector).getAttribute("src");
  ...
  const toastImg = myToast.querySelector(itemImageSelector);
  toastImg.setAttribute("src", thumnailUrl);
  toastImg.setAttribute("alt", title.toLowerCase().replaceAll(" ", "-"));
  myToast.querySelector(toastTitleSelector).textContent = title;
  myToast.querySelector(toastQtySelector).textContent = `Quantity - x${product.quantity}`;

  if (cart.hasOwnProperty(product.id)) {
    product.quantity = cart[product.id].quantity + 1;
    myToast.querySelector(toastQtySelector).textContent = `Quantity - x${cart[product.id].quantity + 1}`;
  }
  ...
  if (typeof item === "string") {
    switch (item) {
      ...
      case "In 12 installments":
        ...
        if (loadPreviousFilters)
          eventTarget = arrayInactiveBtns.filter((btn) => btn.innerText === "In 12 installments")[0];
      ...
      case "In 6 installments":
        ...
        if (loadPreviousFilters)
          eventTarget = arrayInactiveBtns.filter((btn) => btn.innerText === "In 6 installments")[0];
      ...
      case "Special offer":
        ...
        if (!objFilters.promotions.includes("Special offer"))
          objFilters.promotions = [...objFilters.promotions, "Special offer"];
      ...
      case "Free shipping":
        ...
        if (!objFilters.delivery.includes("Free shipping"))
          objFilters.delivery = [...objFilters.delivery, "Free shipping"];
      ...
      case "Withdrawal in person":
        apiShopItems = Object.values(apiShopItems).filter((obj1) => !obj1.hasFreeShipping);
        if (!objFilters.delivery.includes("Withdrawal in person"))
          objFilters.delivery = [...objFilters.delivery, "Withdrawal in person"];
        if (loadPreviousFilters)
          eventTarget = arrayInactiveBtns.filter((btn) => btn.innerText === "Withdrawal in person")[0];
    ...
    if (item.startsWith("CustomPrice:")) {
      ...
      apiShopItems = Object.values(apiShopItems).filter(
        (obj1) => obj1.price >= parseInt(bothPricesArray[0]) && obj1.price <= parseInt(bothPricesArray[1]),
      );
    ...
    if (item.startsWith("CustomDiscount:")) {
      ...
      apiShopItems = Object.values(apiShopItems).filter(
        (obj1) => obj1.discount >= parseInt(bothDiscountsArray[0]) && obj1.discount <= parseInt(bothDiscountsArray[1]),
      );
    ...
    if (item.startsWith("CustomSearch:")) {
      ...
      apiShopItems = Object.values(apiShopItems).filter((obj1) =>
        obj1.title.toLowerCase().startsWith(text.toLowerCase()),
      );
...
function searchFilter(event) {
  ...
  if (searchInput.value !== "") {
    apiShopItems = Object.values(apiShopItems).filter((obj1) =>
      obj1.title.toLowerCase().startsWith(searchInput.value.toLowerCase()),
    );
...
function priceAndDiscountFilter(event, minContainer, maxContainer, filterProperty, productProperty, stringTemplate) {
  ...
  if (minContainer.value !== "" && maxContainer.value !== "") {
    apiShopItems = Object.values(apiShopItems).filter(
      (obj1) =>
        obj1[productProperty] >= parseInt(minContainer.value) && obj1[productProperty] <= parseInt(maxContainer.value),
    );
...
function renderCartSubmenu(visible = true) {
  ...
  if (cart) {
    ...
    //total prices
    const nDiscountedPrices = Object.values(products).reduce(
      (acc, {price, hasDiscount, quantity}) => acc + (price - (price * hasDiscount) / 100) * quantity,
      0,
    );
    ...
    products.forEach((product) => {
      ...
      //price
      templateCartSubmenuProduct.querySelector(submenuProductPriceSelector).textContent = `$${
