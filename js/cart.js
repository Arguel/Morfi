...
function renderCartFooter(arrayItems) {
  ...
  const {rfinalPrice, rshipping, rhasfreeshipping, rpaymentMethod, rcoupondiscount, rcoupondiscountpercentage} =
    footerCalculator(arrayItems);
  ...
  //Coupon discounts:
  templateCartFooter.querySelector(discountsLabelSelector).textContent = `-$${rcoupondiscount.toFixed(
    2,
  )} (${rcoupondiscountpercentage}%)`;
  //Total:
  templateCartFooter.querySelectorAll(finalPriceSelector)[3].textContent = (
    rfinalPrice +
    rshipping +
    rpaymentMethod -
    rcoupondiscount
  ).toFixed(2); //the last item
  ...
  cartMainContainer
    .querySelector(`img[data-method="${checkoutStatus.chosenPaymentMethod}"]`)
    .parentNode.classList.add("border", "border-2", "border-primary");
...
function updateCartContent(element) {
  ...
  //Coupon discounts:
  newCartFooter.querySelector(discountsLabelSelector).textContent = `-$${rcoupondiscount.toFixed(
    2,
  )} (${rcoupondiscountpercentage}%)`;
  //Total:
  newCartFooter.querySelectorAll(finalPriceSelector)[3].textContent = (
    rfinalPrice +
    rshipping +
    rpaymentMethod -
    rcoupondiscount
...
function checkCoupon() {
  ...
  if (validCoupons[coupon.value.toLowerCase()]) {
    ...
    couponContainer.querySelector(firstCouponSpanSelector).textContent =
...
function buildDiscountTag(mainTag) {
  ...
  spanRemove.addEventListener("click", (e) => {
    ...
    footerSummary.querySelectorAll(finalPriceSelector)[3].textContent = (
      rfinalPrice +
      rshipping +
      rpaymentMethod -
      rcoupondiscount
  ...
  const {rfinalPrice, rshipping, rpaymentMethod, rcoupondiscount, rcoupondiscountpercentage} =
    footerCalculator(itemsToBuy);
  document.querySelector(discountsLabelSelector).textContent = `-$${rcoupondiscount.toFixed(
    2,
  )} (${rcoupondiscountpercentage}%)`;
  footerSummary.querySelectorAll(finalPriceSelector)[3].textContent = (
    rfinalPrice +
